...
let pages = dv.pages('"Shadow Ledger"').where(t => t.Date > data.StartDate).where(t => t.Herculean > 0);
let comp = dv.func.default(dv.func.sum(pages.Herculean), 0);
...
### Table
```dataviewjs
let pages = dv.pages('"Shadow Ledger"').where(t => t.Herculean > 0);
